...
| <a name="input_redis_custom_password"></a> [redis\_custom\_password](#input\_redis\_custom\_password) | A custom password for redis which will be used over the generated one. | `string` | `null` | no |
| <a name="input_redis_service_port"></a> [redis\_service\_port](#input\_redis\_service\_port) | The external service port for Redis | `number` | `6379` | no |
| <a name="input_redis_volume_options"></a> [redis\_volume\_options](#input\_redis\_volume\_options) | The redis volume driver with its options. | <pre>object({<br>    driver_name    = optional(string)<br>    driver_options = optional(map(string))<br>    labels         = optional(map(string))<br>    no_copy        = optional(bool)<br>  })</pre> | `{}` | no |
