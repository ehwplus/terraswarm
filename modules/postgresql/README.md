...
| <a name="input_postgres_volume_options"></a> [postgres\_volume\_options](#input\_postgres\_volume\_options) | The PostgreSQL data volume driver with its options. | <pre>object({<br>    driver_name    = optional(string)<br>    driver_options = optional(map(string))<br>    labels         = optional(map(string))<br>    no_copy        = optional(bool)<br>  })</pre> | `{}` | no |
